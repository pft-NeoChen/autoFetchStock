...
- [x] [TASK-103] 擴充 AppConfig 新聞設定欄位
  - **複雜度**：S
  - **依賴**：TASK-101
...
- [x] [TASK-110] 實作 NewsFetcher RSS 抓取
  - **複雜度**：M
...
- [x] [TASK-111] 實作 NewsFetcher 全文抓取
  - **複雜度**：M
  - **依賴**：TASK-110
  - **需求對應**：REQ-226
  - **檔案**：`src/news/news_fetcher.py`
...
- [x] [TASK-112] 實作 NewsFetcher 分類抓取整合
  - **複雜度**：M
  - **依賴**：TASK-111
...
- [x] [TASK-113] 實作個股新聞抓取與 TW/US 自動判斷
  - **複雜度**：M
  - **依賴**：TASK-112
...
- [x] [TASK-120] 實作 NewsSummarizer Gemini API 整合
  - **複雜度**：M
  - **依賴**：TASK-103
...
- [x] [TASK-130] 實作 NewsProcessor 主流程
  - **複雜度**：L
...
- [x] [TASK-131] 實作 NewsProcessor 我的最愛整合
  - **複雜度**：M
  - **依賴**：TASK-130
...
- [x] [TASK-133] 擴充 DataStorage 新聞儲存
  - **複雜度**：M
  - **依賴**：TASK-102
...
- [x] [TASK-140] 擴充 Scheduler 新增新聞排程
  - **複雜度**：M
...
- [x] [TASK-141] 擴充 AppController 初始化新聞模組
  - **複雜度**：S
  - **依賴**：TASK-140
  - **需求對應**：REQ-210
...
- [x] [TASK-150] 擴充 layout.py（路由與跑馬燈）
  - **複雜度**：M
  - **依賴**：TASK-102
...
- [x] [TASK-154] 實作主畫面新聞 Tab callback
  - **複雜度**：M
...
- [x] [TASK-155] 實作 /news 頁面 callback
  - **複雜度**：M
...
- [x] [TASK-160] 建立 tests/test_news/ 目錄結構
  - **複雜度**：S
  - **依賴**：TASK-101
  - **需求對應**：REQ-200
...
- [x] [TASK-161] 實作 test_news_models.py
  - **複雜度**：S
...
- [x] [TASK-162] 實作 test_news_fetcher.py
  - **複雜度**：M
...
- [x] [TASK-163] 實作 test_news_summarizer.py
  - **複雜度**：M
...
- [x] [TASK-164] 實作 test_news_processor.py
  - **複雜度**：L
