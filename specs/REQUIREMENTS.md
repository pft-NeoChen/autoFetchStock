...
#### REQ-226 (基礎型 / Ubiquitous)
新聞子模組在取得 RSS 文章列表後，應對每篇文章的原始 URL 發出 HTTP GET 請求以抓取完整全文（方案 B）。全文抓取應使用 BeautifulSoup 解析 HTML 並提取正文段落；若全文抓取失敗（逾時、HTTP 錯誤、解析失敗），應 fallback 使用 RSS 提供的短摘錄（description 欄位）作為摘要輸入，並於 `NewsArticle` 中記錄 `full_text_fetched: bool` 旗標。

#### REQ-227 (基礎型 / Ubiquitous)
新聞子模組在對每篇 `NewsArticle` 進行摘要時，應同時要求摘要後端（Gemini API）判斷該文章與使用者我的最愛清單中哪些股票相關，並將關聯的股票代號列表寫入 `related_stock_ids` 欄位。此標記適用於所有五個分類（`INTERNATIONAL`、`FINANCIAL`、`TECH`、`STOCK_TW`、`STOCK_US`），以支援主畫面的個股新聞過濾功能。
...
#### REQ-300 (事件驅動型 / Event-Driven)
當使用者導覽至 `/news` 路由時，系統應顯示獨立新聞總覽頁面，以分類頁籤（`INTERNATIONAL`、`FINANCIAL`、`TECH`、`STOCK_TW`、`STOCK_US`）呈現全部收集到的新聞，不針對特定股票過濾。

#### REQ-301 (基礎型 / Ubiquitous)
新聞頁面應於頂部導航列新增「新聞」入口，與現有「股票」主畫面並列，支援雙路由切換（`/` 股票主畫面、`/news` 新聞頁面）。

#### REQ-302 (基礎型 / Ubiquitous)
新聞頁面每個分類頁籤應顯示該分類最新一次執行收集到的所有文章，每篇文章呈現：標題、新聞來源、發布時間、繁體中文摘要（200 字以內）、原文連結。

#### REQ-303 (基礎型 / Ubiquitous)
新聞頁面應顯示最後更新時間（對應 `data/news/latest.json` 的執行時間戳記），並提供手動重新整理按鈕，觸發前端重新讀取 `data/news/latest.json`。

### 主畫面個股新聞頁籤（Tab）

#### REQ-310 (事件驅動型 / Event-Driven)
當使用者在主畫面（`/`）查看某支股票時，主內容區應新增第三個頁籤「新聞」，顯示所有 `related_stock_ids` 包含當前股票代號的文章，並依分類（`INTERNATIONAL`、`FINANCIAL`、`TECH`、`STOCK_TW`、`STOCK_US`）以子頁籤分類呈現。

#### REQ-311 (狀態驅動型 / State-Driven)
當主畫面個股新聞頁籤中某分類無與當前股票相關的文章時，該子頁籤應顯示「目前無相關新聞」提示，而非空白頁面。

#### REQ-312 (基礎型 / Ubiquitous)
主畫面個股新聞頁籤的文章呈現格式應與 `/news` 頁面一致（標題、來源、時間、摘要、原文連結）。

### 主畫面新聞跑馬燈

#### REQ-320 (狀態驅動型 / State-Driven)
當主畫面正在查看某支股票且 `data/news/latest.json` 存在時，主畫面底部應顯示新聞跑馬燈，從五個分類各取最新一篇 `related_stock_ids` 包含當前股票代號的文章標題，依序輪播（每則顯示 5 秒）。

#### REQ-321 (事件驅動型 / Event-Driven)
當使用者點擊跑馬燈中的新聞標題時，主畫面應自動切換至「新聞」頁籤並捲動至對應文章。

#### REQ-322 (狀態驅動型 / State-Driven)
當目前查看的股票無任何相關新聞可輪播時，跑馬燈區域應隱藏，不顯示空白占位區塊。
...
| REQ-226 | 新聞來源分類 | 基礎型 | 必要 |
| REQ-227 | 新聞來源分類 | 基礎型 | 必要 |
...
| REQ-282 | 選配功能 | 選配型 | 選配 |
| REQ-283 | 選配功能 | 選配型 | 選配 |
| REQ-300 | UI - 新聞頁面 | 事件驅動型 | 必要 |
| REQ-301 | UI - 新聞頁面 | 基礎型 | 必要 |
| REQ-302 | UI - 新聞頁面 | 基礎型 | 必要 |
| REQ-303 | UI - 新聞頁面 | 基礎型 | 必要 |
| REQ-310 | UI - 個股新聞頁籤 | 事件驅動型 | 必要 |
| REQ-311 | UI - 個股新聞頁籤 | 狀態驅動型 | 必要 |
| REQ-312 | UI - 個股新聞頁籤 | 基礎型 | 必要 |
| REQ-320 | UI - 跑馬燈 | 狀態驅動型 | 必要 |
| REQ-321 | UI - 跑馬燈 | 事件驅動型 | 建議 |
| REQ-322 | UI - 跑馬燈 | 狀態驅動型 | 必要 |
...
**需求 -> 設計 品質門檻**：
- ✓ 所有需求均具有唯一識別碼（REQ-XXX 格式），共計 61 項需求（REQ-200 至 REQ-322），無與現有 REQ-001~REQ-106 衝突
...
- ✓ 涵蓋所有功能領域（排程、新聞分類收集、全文抓取、股票標記、我的最愛整合、摘要、資料儲存、效能、可靠性、可維護性、UI）
